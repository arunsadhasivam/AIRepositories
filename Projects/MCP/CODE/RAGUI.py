...
class RAGUI:
    ...
    def __init__(self,controller:SearchController,service:SearchService):
        ...
        with tab2:
            ...
            try :
                if query and st.button("Search"):
                  response =   self.controller.searchQuery(self,query,search_type)
                  st.write(response)
                if query and st.button("Search Via MCP"):
                  response =   self.controller.routeMCPSearchQuery(self,query,search_type)
                  st.write(response)  
...
if __name__ == "__main__":
    interface = RAGUI(SearchController,SearchService)
    interface.run()
